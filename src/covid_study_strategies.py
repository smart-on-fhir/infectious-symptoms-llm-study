...
from .instructions import simple_instruction, identity_instruction, include_instruction, exclude_instruction, verbose_instruction, join_lines, symptom_list
...
def build_strategies(model):
    ...
    def newStrat(inst, prompt_format : str = None): 
        return Strategy([{"instruction": inst, "prompt_format": prompt_format}], model=model)
    ...
    jsonSchemaInst = join_lines([
        ...
        "Format your response using only the following JSON schema: {" + ', '.join(f"\"{symptom}\": boolean" for symptom in symptom_list) +"}. Each JSON key should correspond to a symptom, and each value should be true if that symptom is indicated in the clinical note; false otherwise.",
    ...
    jsonFixInst = join_lines([
        ...
        "Otherwise, remove all non-JSON information. Then format the text so that it's valid, parsable JSON that conforms to the following JSON schema: {" + ', '.join(f"\"{symptom}\": boolean" for symptom in symptom_list) +"}",
        "Never explain yourself, and only reply with JSON."
    ])
